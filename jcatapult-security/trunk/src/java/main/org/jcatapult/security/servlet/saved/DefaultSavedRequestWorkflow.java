...
/**
 * <p> This class provides saved request handling logic by implementing the {@link PostLoginHandler} and {@link
 * NotLoggedInHandler}. </p> <p/> <h3>Login handling</h3> <p> This class handles authorization failures due to no user
 * being logged into the application. In this case, this handler saves the request into the session so that once the
 * user logs in the request can be re-executed. Once the request is saved, this class redirects to the login URI. This
 * URI is set using the configuration parameter named <code>jcatapult.security.login.uri</code>. This parameter defaults
 * to <code>/login</code>. </p> <p/> <h3>Post login handling</h3> <p> This class handles successful logins by first
 * checking for a saved request. If there is a saved request, it is re-executed using a redirect. If there is no saved
 * request, this class passes the control to a struts action. The URI of the struts action that is invoked is controlled
 * by the configuratin parameter <code>jcatapult.security.login.successful-uri</code>. This parameter defaults to
 * <b>/login-success</b>. </p>
 *
