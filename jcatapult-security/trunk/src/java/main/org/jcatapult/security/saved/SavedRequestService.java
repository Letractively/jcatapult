...
public interface SavedRequestService {
    ...
    /**
     * The second step of the saved request processing. This fetches a saved request from the session
     * and then handles determining what the saved requests URI was. It also sets up the saved request
     * so that after a the caller has redirected to the URI saved request URI, which is the return
     * value of this method, the saved request workflow will handle the final leg of the saved
     * request processing.
     *
    ...
    /**
     * The final step in the saved request process. This occurs after the redirect has happened and
     * the saved request workflow needs to mock out the savaed request. This method returns the
     * HttpServletRequest that mocks the saved request.
     *
