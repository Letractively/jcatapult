...
/**
 * <p> This class handles login exceptions by creating a Struts request that can be handled by a Struts action. In most
 * cases the URI for login is <code>/jcatapult-security-check</code>. This class allows you to use a different URI to
 * invoke a Struts action for handling failed logins. The configuration parameter that controls the failed login URI is
 * <code>jcatapult.security.login.failed-uri</code> The default for this configuration parameter is
 * <b>/login-failed</b>. </p> <p/> <p> In addition, this class always places the exception that was thrown into the HTTP
 * request under the key <code>jcatapult.security.login.exception</code>. </p>
 *
