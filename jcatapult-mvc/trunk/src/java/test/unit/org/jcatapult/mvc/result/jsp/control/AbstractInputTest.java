...
public class AbstractInputTest extends AbstractControlTest {
    ...
    protected ContainerResolver makeContainerResolver(String name) {
        ...
        EasyMock.expect(containerResolver.getRealPath("WEB-INF/control-templates/dynamic-attributes_en_US.ftl")).andReturn("src/ftl/main/WEB-INF/control-templates/dynamic-attributes.ftl");
        if (label) {
    ...
    protected void run(AbstractInput input, Object action, String template, String bundle, String key,
        ...
        input.setServices(Locale.US, request, ais, fms);
        input.setMessageProvider(mp, ms);
