...
/**
 * <p> This class is a base class that contains helpful methods for setting up and tearing down JPA. </p>
 *
...
public abstract class JPABaseTest extends JCatapultBaseTest {
  ...
  /**
   * Allows sub-classes to setup different databases. The default is to setup a database connection to the database
   * that has the same name as the project (dashes are replaced by underscores) and using the MySQL connection.
   *
  ...
  /**
   * Allows sub-classes to setup different databases. This is a data source so that PostgreSQL or Oracle database can
   * be setup.
   *
